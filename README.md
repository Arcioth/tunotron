...
[![Tests](https://img.shields.io/badge/tests-9%20passed%20%7C%200%20warnings-brightgreen?style=for-the-badge)](#-automated-test-suite)
...
- **💤 0.0% Idle CPU:** Tokio reactor sleeps inside `epoll_wait`. The progress bar ticker automatically deactivates when playback is paused or stopped.
- **⚡ Throttled 4 Hz Render Pipeline:** mpv emits playback timestamps at 30–50 Hz; Tunotron decouples timestamps from redraws, capping screen rendering to 4 Hz and saving massive CPU on both the player and your terminal emulator.
- **🚀 Sub-Millisecond Folder Navigation:** File listing and tag parsing are decoupled. Browsing huge music libraries is instantaneous (`< 1ms`); audio tags and durations are extracted asynchronously in background worker batches.
...
Following a comprehensive systems audit in `v0.2.0`, Tunotron eliminated hot-path render loops and memory churn:

| Performance Metric | Unoptimized Baseline (`v0.1.1`) | Optimized Core (`v0.2.0`) | Improvement |
| :--- | :--- | :--- | :--- |
...
Tunotron includes automated unit tests covering security jails, PRNG determinism, playback loop modes, keychord normalization, and background metadata patching:
